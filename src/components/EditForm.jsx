...
function EditForm() {
    ...
    useEffect(()=>{
        ...
        axios.get(`${API}/v1/contacts/${id}`,{headers}).then(res=>{
            ...
            setFirstName(res?.data?.contact?.firstName);
            setLastName(res?.data?.contact?.lastName);
            setPhone(res?.data?.contact?.phone);
            setName(res?.data?.contact?.fullNameLowerCase);
            setDOB(res?.data?.contact?.dateOfBirth);
            setAddress(res?.data?.contact?.address1);
            setCity(res?.data?.contact?.city);
            setState(res?.data?.contact?.state);
            setCountry(res?.data?.contact?.country);
            setCompany(res?.data?.contact?.company);
            setWebsite(res?.data?.contact?.website);
            setValue(res?.data?.contact?.customField[0]?.value)
   ...
   <div className="container-fluid">
                <div className="row">
                    <div className="col-12  flex justify-center">
                        <h1 className=" font-bold text-2xl my-3">Form for Edit</h1>
                    </div>
                    <div className="col-12  flex justify-center">
                        <form className=" w-11/12 md:w-6/12" onSubmit={submit}>
                            ...
                            <button type="submit" className="btn bg-[#6687bd] hover:bg-[#6687bd] text-white mb-4">Edit</button>
                        </form>
                    </div>
                </div>
            </div>
